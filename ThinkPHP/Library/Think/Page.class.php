...
class Page {
	...
	protected $config = array (
			...
			'next' => '下一页',
			'first' => '首页',
			'last' => '<span id="lastspan">尾页</span>',
			'theme' => '<ul><li><span> %totalRow% %header% %nowPage%/%totalPage% 页</span></li> %first% %upPage%  %linkPage%  %downPage% %end%</ul>' 
	) // theme %prePage%上5页，%nextPage%下五页，%first%第一页，%end%最后一页
;
	// 默认分页变量名
	protected $varPage;
	...
	public function show() {
		...
		if ($upRow > 0) {
			$upPage = "<li><a class='prev' href='" . str_replace ( '__PAGE__', $upRow, $url ) . "'>" . $this->config ['prev'] . "</a></li>";
		} else {
			$upPage = '';
		}
		
		if ($downRow <= $this->totalPages) {
			$downPage = "<li><a class='next' href='" . str_replace ( '__PAGE__', $downRow, $url ) . "'>" . $this->config ['next'] . "</a></li>";
		} else {
			$downPage = '';
		}
		// << < > >>
